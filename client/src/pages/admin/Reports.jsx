  ...
  return (
    <DashboardLayout title="Admin" nav={ADMIN_NAV}>
      <div className="flex flex-col gap-xl">
        ...
        {status === 'error' && (
          <EmptyState icon="wifi_off" title="Couldn't load reports" description="Please check your connection and try again." />
        )}

        {status === 'ready' && reports.length === 0 && (
          <EmptyState icon="flag" title="No open reports" description="Reports from donors and organizers will appear here." />
        )}
              ...
              <div key={r._id} className="flex items-center justify-between py-lg gap-md flex-wrap">
                <div className="min-w-0">
                  ...
                  <p className="text-[13px] text-text-secondary">
                    {r.targetType} · reported by {r.reporterId?.fullName || 'unknown'} ·{' '}
                    <Link to={targetLink(r)} className="text-primary hover:underline">view target</Link>
                  </p>
                </div>
